...
def main():
    ...
    app_data = DASHBOARD_DIR / "src" / "data"
    app_data.mkdir(parents=True, exist_ok=True)
    (app_data / "trackman.json").write_text(json.dumps(payload), encoding="utf-8")
    report = ROOT / "REPORT.md"
    if report.exists():
        (app_data / "report.md").write_text(report.read_text(encoding="utf-8"), encoding="utf-8")
    print(f"\nWrote {DATA_DIR / 'sessions.json'}, {DATA_DIR / 'combines.json'}, {app_data / 'trackman.json'}")
    build_single_file()
    try:
...
def build_single_file():
    """Best effort: rebuild the shareable single-file dashboard (dashboard/trackman-dashboard.html)
    via `npm run build:single`. Skipped when node_modules is missing (Vercel builds the site itself)."""
    import shutil
    import subprocess

    if not (DASHBOARD_DIR / "node_modules").exists():
        print("Single-file dashboard not rebuilt (run `npm install` in dashboard/ to enable).")
        return
    npm = shutil.which("npm.cmd") or shutil.which("npm")
    if not npm:
        return
    try:
        r = subprocess.run([npm, "run", "build:single", "--silent"], cwd=DASHBOARD_DIR, capture_output=True, text=True, timeout=300)
        if r.returncode == 0:
            print(f"Wrote {DASHBOARD_DIR / 'trackman-dashboard.html'}")
        else:
            print("Single-file build failed:\n" + (r.stderr or r.stdout)[-2000:])
    except Exception as e:
        print(f"Single-file build skipped: {e}")
